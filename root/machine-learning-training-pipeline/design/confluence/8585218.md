...
While **parameters** (like weights and biases) are learned automatically during model training, **hyperparameters** are structural configurations set _before_ training begins.
...
Defined by the engineer / optimization loop
...
Neural network weights, SVM support vectors

 | 

Learning rate, batch size, dropout rate, tree depth
...
Minimize the training loss function

 | 

Maximize validation metrics (e.g., F1-score, AUC)
...
-   **Cons:** Suffer drastically from the _curse of dimensionality_. If you have 5 parameters with 5 values each, that requires $5^5 = 3125$ training runs.
